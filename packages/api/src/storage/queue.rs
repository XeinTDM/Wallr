                ...
                if let Ok((_, json)) = res {
                    if let Ok(payload) = serde_json::from_str::<UploadJobPayload>(&json) {
                        ...
                        if let Ok(bytes) = bytes_res {
                            if !bytes.is_empty() {
                                println!("⚙️ Processing upload job for {} ({} bytes)", payload.id, bytes.len());
                                process_upload_job(payload, bytes).await;
                                println!("✅ Finished processing upload job.");
                            } else {
                                let _ = crate::storage::wallpapers::core::update_upload_job_status(&payload.id, "failed", Some("Upload bytes missing or empty in Redis")).await;
                            }
                        } else {
