...
pub async fn push_upload_job(payload: &UploadJobPayload, bytes: &[u8]) -> anyhow::Result<()> {
    ...
    // Store the raw bytes permanently so they don't expire before processing or reprocessing
    crate::storage::save_image_bytes(&payload.id, bytes, "raw", "bin").await?;
    ...
    loop {
        if let Ok(pool) = crate::storage::get_redis_pool() {
            if let Ok(mut conn) = pool.get().await {
                ...
                {
                    let bytes_res = crate::storage::get_image_bytes(&payload.id, "raw", "bin").await;
                    ...
                    if let Ok(bytes) = bytes_res {
                        if !bytes.is_empty() {
                            ...
                            process_upload_job(payload.clone(), bytes).await;
                            println!("✅ Finished processing upload job.");
                            // Clean up the raw bytes on success
                            if let Ok(status) = crate::storage::wallpapers::core::get_upload_status(&payload.id).await {
                                if status.map_or(false, |s| s.status == "completed") {
                                    let _ = crate::storage::delete_image_bytes(&payload.id, "raw", "bin").await;
                                }
                            }
                        } else {
                            let _ = crate::storage::wallpapers::core::update_upload_job_status(
                                &payload.id,
                                "failed",
                                Some("Upload bytes missing or empty in storage"),
                            )
                            .await;
                        }
                    } else {
                        let _ = crate::storage::wallpapers::core::update_upload_job_status(
                            &payload.id,
                            "failed",
                            Some("Upload bytes not found in storage"),
                        )
                        .await;
                    }
                }
