...
use api::{get_dmca_claims, resolve_dmca_claim};
use dioxus::prelude::*;
use lucide_dioxus::{ArrowLeft, Check, ShieldAlert, Trash2};

#[component]
pub fn AdminDmca() -> Element {
    ...
    #[allow(unused_mut)]
    let mut claims_res = use_server_future(move || {
        let f = status_filter();
        async move {
            let status = if f == "all" { None } else { Some(f) };
            get_dmca_claims(status).await
        }
    })?;
    ...
    let resolve_action = move |claim_id: String, action: String| {
        spawn(async move {
            if let Ok(_) = resolve_dmca_claim(claim_id, action).await {
                ...
                toaster.error("Failed to resolve DMCA claim");
            }
        });
    };
    ...
    rsx! {
        div { class: "container", style: "padding: 120px 0 80px;",
            ...
            div { style: "display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 32px;",
                div {
                    h1 { style: "font-size: 32px; font-weight: 900; margin: 0 0 8px 0; display: flex; align-items: center; gap: 12px;",
                        ShieldAlert { size: 32, color: "#ef4444" }
                        "DMCA Claims"
                    }
                    p { style: "color: var(--text-secondary); margin: 0;",
                        "Review and resolve DMCA takedown requests."
                    }
                }
            ...
            div {
                ...
                match claims_res() {
                                        ...
                                        div {
                                            ...
                                            div { style: "flex: 1; display: flex; flex-direction: column; justify-content: center;",
                                                div { style: "display: flex; justify-content: space-between; margin-bottom: 8px;",
                                                    ...
                                                    {
                                                        let date_str = claim.created_at.format("%Y-%m-%d %H:%M").to_string();
                                                        rsx! {
                                                            span { style: "font-size: 13px; color: var(--text-muted);", "{date_str}" }
                                                        }
                                                    }
                                                }
                                                div { style: "margin-bottom: 12px;",
                                                    strong { "Wallpaper ID: " }
                                                    Link {
                                                        to: Route::WallpaperDetail {
                                                            id: claim.wallpaper_id.clone(),
                                                        },
                                                        style: "color: var(--accent-primary); text-decoration: none;",
                                                        "{claim.wallpaper_id}"
                                                    }
                                                }
                                                if let Some(orig_url) = claim.original_url.clone() {
                                                    div { style: "margin-bottom: 12px;",
                                                        strong { "Original Work URL: " }
                                                        a {
                                                            href: "{orig_url}",
                                                            target: "_blank",
                                                ...
                                                div { style: "color: var(--text-secondary); font-size: 14px; margin-bottom: 12px; background: rgba(255,255,255,0.03); padding: 12px; border-radius: 8px; border-left: 2px solid #ef4444;",
                                                    "{claim.description}"
                                                }
                                                div { style: "display: flex; justify-content: space-between; align-items: center;",
                                                    div {
                                                        span { style: "font-size: 12px; padding: 4px 8px; border-radius: 4px; background: rgba(255,255,255,0.05); color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.05em; font-weight: 600;",
                                                            "{claim.status}"
                                                        }
                                                    }
                    ...
                    Some(Err(e)) => {
                        rsx! {
                            div { style: "color: #ef4444; padding: 24px; background: rgba(239, 68, 68, 0.1); border-radius: 12px;",
                    ...
                    None => {
                        rsx! {
                            div { style: "display: flex; flex-direction: column; gap: 16px;",
                                for _ in 0..3 {
                                    div { style: "height: 120px; background: rgba(255,255,255,0.02); border-radius: 16px; animation: pulse 2s infinite;" }
                                }
                            }
                        }
                    }
