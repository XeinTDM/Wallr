...
pub fn Profile() -> Element {
    ...
    let uploads = use_server_future(move || async move { api::get_user_uploads(0, 100).await })?;
    let collections = use_server_future(move || async move { get_my_collections().await })?;
    let favorites =
        use_server_future(move || async move { api::get_user_favorites(None::<String>, 100).await })?;
    let analytics = use_server_future(move || async move { api::get_creator_analytics().await })?;
        ...
        div {
            div {
                ...
                div { class: "container", style: "padding-bottom: 80px;",
                    ...
                    div { style: "display: flex; gap: 32px; margin-bottom: 48px; border-bottom: 1px solid rgba(255,255,255,0.1);",
                        {
                            render_profile_tab(
                                i18n.t("profile_tab_favorites"),
                                favorites_count,
                                active_tab() == "favorites",
                                move |_| active_tab.set("favorites".into()),
                            )
                        }
                        {
                            render_profile_tab(
                                i18n.t("profile_tab_uploads"),
                                uploads_count,
                                active_tab() == "uploads",
                                move |_| active_tab.set("uploads".into()),
                            )
                        }
                        {
                            render_profile_tab(
                                i18n.t("profile_tab_collections"),
                                collections_count,
                                active_tab() == "collections",
                                move |_| active_tab.set("collections".into()),
                            )
                        }
                        {
                            render_profile_tab(
                                i18n.t("profile_tab_analytics"),
                                None,
                                active_tab() == "analytics",
                                move |_| active_tab.set("analytics".into()),
                            )
                        }
                    }
                            ...
                            "favorites" | "uploads" => rsx! {
                                div {
        ...
        
                                    {
                                        ...
                                        if is_loading {
                                            rsx! {
                                                div { style: "grid-column: 1 / -1; display: flex; justify-content: center; padding: 80px 20px;",
                                                    crate::LoadingScreen {}
                                                }
                                            }
                                            ...
                                            rsx! {
                                                for wp in render_list {
                                                    WallpaperCard { key: "{wp.id}", wallpaper: wp.clone() }
                                                }
                                            }
                                        }
                                    }
                                    ...
                                    match collections() {
                                        Some(Ok(list)) if !list.is_empty() => rsx! {
                                            ...
                                            div {
                                                ...
                                                span { style: "margin-top: 16px; font-weight: 600; color: var(--text-secondary); font-size: 18px;",
                                                    "New Collection"
                                                }
                                            }
                                        },
                                        Some(Ok(_)) => rsx! {
                                            div { style: "grid-column: 1 / -1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 80px 20px; text-align: center; background: rgba(255,255,255,0.02); border-radius: 24px; border: 1px dashed rgba(255,255,255,0.1);",
                                                ...
                                                h3 { style: "font-size: 20px; font-weight: 700; margin-bottom: 8px;",
                                                    "{i18n.t(\"profile_no_collections\")}"
                                                }
                                                p { style: "color: var(--text-secondary); margin-bottom: 24px;",
                                                    "{i18n.t(\"profile_organize_collections\")}"
                                                }
                                                button {
                                                    class: "glow-hover",
                                        ...
                                        _ => rsx! {
                                            div { "{i18n.t(\"profile_loading_collections\")}" }
                                        },
                                    }
                                }
                            },
                            "analytics" => rsx! {
                                div { class: "analytics-dashboard",
                                    match analytics() {
                                            ...
                                            div { style: "display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 24px;",
                                                div {
                                                    ...
                                                    lucide_dioxus::Image {
                                                        ...
                                                        style: "margin-bottom: 16px; margin-inline: auto;",
                                                    }
                                                    h4 { style: "font-size: 16px; color: var(--text-secondary); margin-bottom: 8px;",
                                                        "{i18n.t(\"profile_total_uploads\")}"
                                                    }
                                                ...
                                                div {
                                                    class: "glass glow-hover",
                                                    style: "padding: 32px; border-radius: 24px; text-align: center;",
                                                    lucide_dioxus::Download {
                                                        size: 32,
                                                        color: "#10b981",
                                                        style: "margin-bottom: 16px; margin-inline: auto;",
                                                    }
                                                    h4 { style: "font-size: 16px; color: var(--text-secondary); margin-bottom: 8px;",
                                                        "{i18n.t(\"profile_total_downloads\")}"
                                                    }
                                        ...
                                        Some(Err(e)) => rsx! {
                                            div { style: "padding: 80px 20px; text-align: center; color: #ef4444;",
                                                "{i18n.t(\"profile_err_analytics\")}{e}"
                                            }
                                        },
                                        None => rsx! {
                                            div { style: "display: flex; justify-content: center; padding: 80px 20px;", crate::LoadingScreen {} }
                                        },
                                    }
                                }
                            },
                            _ => rsx! {
                                div { "{i18n.t(\"profile_tab_not_found\")}" }
                            },
                        }
                    }
                }
    ...
    rsx! {
        Link {
            ...
            if let Some(url) = &collection.cover_url {
                ...
                div { style: "width: 100%; height: 100%; background: linear-gradient(135deg, rgba(37, 99, 235, 0.2) 0%, rgba(5, 5, 5, 1) 100%); display: flex; align-items: center; justify-content: center;",
            ...
            div { style: "position: absolute; bottom: 0; left: 0; right: 0; padding: 24px; background: linear-gradient(transparent, rgba(0,0,0,0.9));",
                h4 { style: "font-size: 20px; font-weight: 800; color: white; margin-bottom: 4px;",
                    "{collection.name}"
                }
                p { style: "font-size: 14px; color: var(--text-muted);",
                    "{collection.item_count}{i18n.t(\"profile_wallpapers_count\")}"
                }
            }
        }
    }
...
pub fn render_profile_tab(
    ...
    rsx! {
        div {
            ...
            "{label} "
            if let Some(c) = count {
                span { style: "font-size: 14px; opacity: 0.6; margin-left: 4px;", "{c}" }
            }
        }
    }
}
        ...
        div { class: "container", style: "padding-top: 24px;",
            ...
            if props.is_owner {
                Link {
                    ...
                    div { class: "edit-overlay", style: "z-index: 2;",
                        lucide_dioxus::Pen { size: 36, color: "white" }
                    }
                }
            } else {
                div { style: "display: block; width: 100%; height: 320px; border-radius: 24px; overflow: hidden; background: {banner_bg} center/cover no-repeat; position: relative; box-shadow: 0 10px 20px rgba(0,0,0,0.2);",
                    div { style: "position: absolute; inset: 0; background: linear-gradient(to bottom, transparent, var(--bg-primary)); z-index: 1;" }
                }
            }

            div { style: "margin-top: -80px; position: relative; z-index: 10; padding: 0 32px;",
                div { style: "display: flex; flex-direction: column; gap: 24px; margin-bottom: 48px;",
                        ...
                        if props.is_owner {
                            Link {
                                ...
                                img {
                                    ...
                                    src: "{crate::resolve_asset_url(&props.user.pfp_url)}",
                                    style: "width: 100%; height: 100%; object-fit: cover;",
                                }
                                div { class: "edit-overlay pfp-overlay",
                            ...
                            if let crate::app::AuthState::Authenticated(_) = is_auth() {
                                div { style: "display: flex; gap: 12px; margin-bottom: 16px;",
                                    button {
                                        class: "glass glow-hover",
                                        style: format!(
                                            "padding: 10px 24px; border-radius: 12px; color: {}; font-weight: 600; font-size: 14px; border: 1px solid {}; background: {}; cursor: pointer; transition: all 0.2s;",
                                            if is_following() { "var(--text-primary)" } else { "white" },
                                            if is_following() { "rgba(255,255,255,0.2)" } else { "var(--accent-primary)" },
                                        ...
                                        {
                                            let text = if is_following() {
                                                "{i18n.t(\"profile_following_btn\")}"
                                            } else {
                                                "{i18n.t(\"profile_follow_btn\")}"
                                            };
                                            rsx! { "{text}" }
                                        }
                                    }
                    ...
                    div {
                        h1 { style: "font-size: 40px; font-weight: 900; margin-bottom: 8px; letter-spacing: -0.02em;",
                            "{props.user.name}"
                        }
                        ...
                        if let Some(bio) = &props.user.bio {
                            p { style: "color: var(--text-muted); font-size: 15px; margin-top: 16px; line-height: 1.5; max-width: 600px;",
                                "{bio}"
                            }
                        }
