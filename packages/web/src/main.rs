use dioxus::prelude::*;
use ui::{DropdownSection, ExploreDropdown, Navbar, Theme, Toast, ToastContainer};
    ...
    #[cfg(feature = "server")]
    {
        ...
        async fn download_handler(
            ...
            let source_path = if format == "avif" && width.is_none() {
                format!("packages/web/assets/uploads/{}_master.avif", id)
            } else {
                format!("packages/web/assets/uploads/{}_master.jpg", id)
            };
            ...
            let data = match tokio::fs::read(&source_path).await {
                Ok(d) => d,
                Err(_) => {
                    let fallback_path = format!("packages/web/assets/uploads/{}_master.jpg", id);
        ...
        dioxus::serve(|| async move {
            ...
            let app = router
                .nest_service(
                    "/assets/uploads",
                    tower_http::services::ServeDir::new("packages/web/assets/uploads"),
                )
...
fn WebNavbar() -> Element {
    ...
    let _auth_resource = use_resource(move || async move {
        if let Ok(Some(u)) = api::get_current_user().await {
            ...
            if let Ok(ids) = api::get_all_user_favorite_ids().await {
                *ui::FAVORITED_IDS.write() = std::collections::HashSet::from_iter(ids);
            }
        } else {
            user.set(AuthState::Unauthenticated);
            ui::FAVORITED_IDS.write().clear();
        }
    });
    ...
    let _scroll_handle = use_hook(move || {
        ...
        {
            std::rc::Rc::new(None::<gloo_events::EventListener>)
        }
    });
