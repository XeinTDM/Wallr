...
use dioxus::prelude::*;
use lucide_dioxus::{Users, Image as ImageIcon, Download, Heart, Activity, ShieldAlert, ArrowRight, ClipboardList};
...
pub fn Admin() -> Element {
    ...
    let (is_allowed, user_role) = match auth_state() {
        ...
        AuthState::Loading => return rsx! { ui::LoadingScreen {} },
    ...
    let mut stats_res = use_resource(move || async move {
        get_admin_stats().await
    });

    let mut audit_res = use_resource(move || async move {
        get_audit_logs(10).await
    });
