...
pub fn Login() -> Element {
    ...
    let mut user_signal = use_context::<Signal<crate::AuthState>>();
            ...
            match api::login(email(), password()).await {
                Ok(_) => {
                    if let Ok(Some(u)) = api::get_current_user().await {
                        user_signal.set(crate::AuthState::Authenticated(u));
                    }
                    nav.push(Route::Home {});
                    return;
                }
    ...
    rsx! {
        div {
            ...
            div {
                ...
                h2 { style: "font-size: 32px; font-weight: 900; margin-bottom: 8px;", "Welcome Back" }
                p { style: "color: var(--text-secondary); margin-bottom: 40px;", "Access your curated wallpaper collection." }
                ...
                div {
                    ...
                    button {
                        ...
                        style: "padding: 16px; border-radius: 16px; display: flex; align-items: center; justify-content: center; gap: 12px; font-weight: 700; color: white; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); cursor: pointer; transition: all 0.2s ease;",
                        GitBranch { size: 20 }
                ...
                p {
                    ...
                    "New to Wallr? "
                    Link { to: Route::Register {}, style: "color: var(--accent-primary); font-weight: 700;", "Create an account" }
                }
