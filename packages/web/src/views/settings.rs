...
pub fn Settings() -> Element {
    ...
    let (real_username, real_email, real_pfp_url, real_bio, real_socials) = match user() {
        ...
        crate::AuthState::Authenticated(u) => (u.name, u.email, u.pfp_url, u.bio, u.social_links),
    };
                                    ...
                                    onchange: move |e| async move {
                                        ...
                                        if !files.is_empty() {
                                            ...
                                            if let Ok(bytes) = file.read_bytes().await
                                                && let Ok(resp) = gloo_net::http::Request::post("/api/upload_media").header("X-Media-Type", "banner").body(bytes.to_vec()).unwrap().send().await {
                                    ...
                                    input {
                                        ...
                                        onchange: move |e| async move {
                                            ...
                                            if !files.is_empty() {
                                                ...
                                                if let Ok(bytes) = file.read_bytes().await
                                                    && let Ok(resp) = gloo_net::http::Request::post("/api/upload_media").header("X-Media-Type", "pfp").body(bytes.to_vec()).unwrap().send().await {
                                    ...
                                    match api::update_profile(name_val, email_val, bio_opt, socials_opt).await {
                                        Ok(_) => {
                                            if let Ok(Some(u)) = api::get_current_user().await {
                                                user_ctx.set(crate::AuthState::Authenticated(u));
                                            }
