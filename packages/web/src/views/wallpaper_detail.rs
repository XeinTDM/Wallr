...
use ui::{LoadingScreen, WallpaperCard, use_toaster};
                            ...
                            match related() {
                                Some(Ok(list)) => rsx! {
                                    for related_wp in list.iter().filter(|w| w.id != wp.id).take(3) {
                                        WallpaperCard {
                                            id: related_wp.id.clone(),
                                            title: related_wp.title.clone(),
                                            author: related_wp.author.clone(),
                                            image_url: related_wp.thumbnail_url.clone(),
                                            tags: related_wp.tags.clone(),
                                            likes: related_wp.likes,
                                            downloads: related_wp.downloads
                                        }
                                    }
                                },
                                _ => rsx! { LoadingScreen {} }
...
fn CommentsSection(wallpaper_id: String, is_wallpaper_author: bool) -> Element {
    ...
    let is_auth = use_context::<Signal<crate::AuthState>>();
    ...
    rsx! {
        div {
            ...
            if let crate::AuthState::Authenticated(u) = is_auth() {
                div {
                    style: "display: flex; gap: 16px; margin-bottom: 32px;",
                    img { src: "{u.pfp_url}", style: "width: 48px; height: 48px; border-radius: 50%; object-fit: cover;" }
                    div {
                        style: "flex: 1;",
                        textarea {
                            class: "glass",
                ...
                if all_comments().is_empty() {
                    ...
                    for comment in all_comments() {
                        div {
                            ...
                            img { src: "{comment.user_pfp}", style: "width: 48px; height: 48px; border-radius: 50%; object-fit: cover;" }
                            div {
                                div {
